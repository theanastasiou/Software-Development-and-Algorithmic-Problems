...
class Hash_Euclidean{
	...
	public:
		...
		void Calc_Euclidean_distance(int buck,std::vector<double> x,std::string &min_item,double &min_distance); // opou x to item_data k opou y ta simeia tu item tu query
		//dimiourgw to bucket
		...
		int Return_Ri();
		std::list<Item*>& Search_Euclidean(std::vector<double> myCl);
		...
		//void Euclidean_AssignToClusters(int bucket,int i,std::vector<Cluster*>& Clusters, double Radius);
};
